...
class RelapseHomeserverOpenIDListenerTests(HomeserverTestCase):
    ...
    def test_openid_listener(self, names: list[str], expectation: str) -> None:
        ...
        hs = self.hs
        hs.listen_http(parse_listener_def(0, config))
        ...
        if expectation == "auth_fail":
            self.assertEqual(channel.code, 401)
        elif expectation == "no_resource":
            self.assertEqual(channel.code, 404)
        else:
            raise ValueError(f"Test setup, unknown expectation: {expectation}")


class FederationReaderOpenIDListenerTests(RelapseHomeserverOpenIDListenerTests):
    def make_homeserver(self, reactor: MemoryReactor, clock: Clock) -> HomeServer:
