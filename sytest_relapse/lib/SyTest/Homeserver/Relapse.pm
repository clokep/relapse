package SyTest::Homeserver::Relapse;
...
sub _init
{
   ...
   # allocations could be moved to SyTest::Homeserver::Relapse::ViaHaproxy::_init
   my $idx = $self->{hs_index};
   $self->{ports} = {
      relapse                  => main::alloc_port( "relapse[$idx]" ),
      relapse_metrics          => main::alloc_port( "relapse[$idx].metrics" ),
...
sub start
{
   ...
   # convert sytest db args onto relapse db args
   ...
   my $config_path = $self->{paths}{config} = $self->write_yaml_file( "config.yaml" => {
        ...
        # We configure relapse to use a TLS cert which is signed by our dummy CA...
        ...
        # ... but relapse currently lacks such an option for non-federation
        ...
        $self->{workers} ? (
            instance_map => {
               "main" => {
                  host => "$bind_host",
                  port => $self->{ports}{relapse_unsecure},
               },
               "event_persister1" => {
                  host => "$bind_host",
   ...
   my @relapse_command = $self->_generate_base_relapse_command();

   $output->diag( "Generating config for port $port" );

   my @config_command = (
      @relapse_command, "--generate-config", "--report-stats=no",
   ...
   return $self->_run_command(
      ...
      $self->_start_relapse( env => $env )
   })->on_done( sub {
      $output->diag("Started relapse $hs_index");
      $self->open_logfile();
   });
}

sub _generate_base_relapse_command
{
   my $self = shift;
   my %params = @_;

   my $app = $params{app} // "relapse.app.homeserver";

   my @relapse_command = ( $self->{python} );

   if( $self->{coverage} ) {
      # Ensures that even --generate-config has coverage reports. This is intentional
      push @relapse_command,
         "-m", "coverage", "run", "--source=$self->{relapse_dir}/relapse", "--rcfile=$self->{relapse_dir}/.coveragerc";
   }
...
sub _start_relapse
{
   ...
   $self->_start_process_and_await_notify(
      setup => [ env => $env ],
      command => \@relapse_command,
      name => "relapse-$idx-master",
   );
}
...
sub generate_listeners
{
   ...
   return @listeners,
      {
         type         => "metrics",
         port         => $self->{ports}{relapse_metrics},
         bind_address => $bind_host,
      };
}

sub wrap_relapse_command
{
   my $self = shift;
   return @_;
}
...
sub open_logfile
{
   ...
   $self->add_child(
      $self->{log_stream} = IO::Async::FileStream->new(
         ...
         on_read => $self->_capture_weakself( 'on_relapse_read' ),
      )
   );
}
...
sub rotate_logfile
{
   ...
   try_repeat {
     ...
     while => sub { !shift->get },
     otherwise => sub { die "Timed out waiting for relapse to recreate its log file" };
}
...
package SyTest::Homeserver::Relapse::Direct;
use base qw( SyTest::Homeserver::Relapse );
...
package SyTest::Homeserver::Relapse::ViaHaproxy;
use base qw( SyTest::Homeserver::Relapse );
...
sub _init
{
   ...
   my $idx = $self->{hs_index};
   $self->{ports}{relapse_unsecure} = main::alloc_port( "relapse[$idx].unsecure" );
...
sub _check_db_config
{
   ...
   $config{type} eq "pg" or die "Relapse can only run against postgres when in worker mode";
...
sub _start_relapse
{
   ...
   my @base_relapse_command = $self->_generate_base_relapse_command();
   my $idx = $self->{hs_index};

   $self->_start_process_and_await_notify(
      setup => [ env => $env ],
      command => \@base_relapse_command,
      name => "relapse-$idx-master",
   )->then( sub {
      Future->needs_all(
         map {
            ...
            my @command = $self->_generate_base_relapse_command( app => $worker_app );
            push @command, "--config-path" => $config_file;

            $self->{output}->diag("Starting relapse $idx worker $worker_name");

            $self->_start_process_and_await_notify(
               setup => [ env => $env ],
               command => \@command,
               name => "relapse-$idx-$worker_name",
            )->then( sub {
               $self->{output}->diag("Started relapse $idx worker $worker_name");
...
sub start
{
   ...
   return $self->SUPER::start->then( sub {
      # We know relapse has started, so lets steal its SSL keys
...
frontend http-in
    ...
    use_backend %[path,map_reg($self->{paths}{get_path_map_file},relapse)] if has_get_map METH_GET

    use_backend %[path,map_reg($self->{paths}{path_map_file},relapse)]

backend relapse
    server relapse ${bind_host}:$ports->{relapse_unsecure}
...
^/_relapse/admin/v1/purge_media_cache$     media_repository
^/_relapse/admin/v1/room/.*/media.*$       media_repository
^/_relapse/admin/v1/user/.*/media.*$       media_repository
^/_relapse/admin/v1/media/.*$              media_repository
^/_relapse/admin/v1/quarantine_media/.*$   media_repository
^/_relapse/admin/v1/users/.*/media$        media_repository
