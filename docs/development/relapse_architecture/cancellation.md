...
before Relapse produces a response. To avoid wasting resources, Relapse
can cancel request processing for select endpoints marked with the
`@cancellable` decorator.

Relapse makes use of Twisted's `Deferred.cancel()` feature to make
...
### Deferred.cancel()
When Relapse starts handling a request, it runs the async method
...
When a client disconnects early, Relapse checks for the presence of the
...
Most functions in Relapse are relatively straightforward from a
