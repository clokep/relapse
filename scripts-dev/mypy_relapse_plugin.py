    ...
    def get_method_signature_hook(
        ...
        if fullname.startswith(
            (
                ...
                "relapse.util.caches.descriptors._LruCachedFunction.__call__",
            )
        ):
            return cached_function_method_signature

        if fullname in (
            "relapse.util.caches.descriptors._CachedFunctionDescriptor.__call__",
            "relapse.util.caches.descriptors._CachedListFunctionDescriptor.__call__",
        ):
...
# Types defined in Relapse which are known to be immutable.
IMMUTABLE_CUSTOM_TYPES = {
    "relapse.relapse_rust.acl.ServerAclEvaluator",
    "relapse.relapse_rust.push.FilteredPushRules",
...
def plugin(version: str) -> Type[RelapsePlugin]:
    ...
    # string.
    #
    # However, since we pin the version of mypy Relapse uses in CI, we don't
    # really care.
    return RelapsePlugin
