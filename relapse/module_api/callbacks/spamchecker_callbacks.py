...
class SpamCheckerModuleApiCallbacks:
    ...
    def __init__(self, hs: "relapse.server.HomeServer") -> None:
    ...
    async def check_event_for_spam(
        self, event: "relapse.events.EventBase"
        ...
        for callback in self._check_event_for_spam_callbacks:
            with Measure(self.clock, f"{callback.__module__}.{callback.__qualname__}"):
                ...
                if res is False or res == self.NOT_SPAM:
                    ...
                    # return value `True`
                    return relapse.api.errors.Codes.FORBIDDEN, {}
                elif (
                    isinstance(res, tuple)
                    and len(res) == 2
                    and isinstance(res[0], relapse.api.errors.Codes)
                    and isinstance(res[1], dict)
                ):
                    return res
                elif isinstance(res, relapse.api.errors.Codes):
                    return res, {}
    ...
    async def user_may_send_3pid_invite(
        ...
        Note that if the threepid is already associated with a Matrix user ID, Relapse
