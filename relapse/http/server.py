...
from relapse.util.iterutils import chunk_seq
...
def return_html_error(
    ...
    Handles RedirectException and other CodeMessageExceptions (such as RelapseError)
...
def wrap_async_request_handler(
    ...
    where "request" must be a RelapseRequest.
    ...
    async def wrapped_async_request_handler(
        self: "_AsyncResource", request: "RelapseRequest"
    ) -> None:
...
class _AsyncResource(resource.Resource, metaclass=abc.ABCMeta):
    ...
    async def _async_render_wrapper(self, request: "RelapseRequest") -> None:
...
def set_cors_headers(request: "RelapseRequest") -> None:
