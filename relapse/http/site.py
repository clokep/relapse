...
class RelapseRequest(Request):
    """Class which encapsulates an HTTP request to relapse.

    All of the requests processed in relapse are of this type.
    ...
    def render(self, resrc: Resource) -> None:
        ...
        self.logcontext = LoggingContext(
            ...
            request=ContextRequest(
                ...
                site_tag=self.relapse_site.site_tag,
        ...
        self.setHeader("Server", self.relapse_site.server_version_string)
    ...
    def _started_processing(self, servlet_name: str) -> None:
        ...
        self.relapse_site.access_logger.debug(
    ...
    def get_client_ip_if_available(self) -> str:
        ...
        In practice, this returns the socket file path on a RelapseRequest if using a
...
class XForwardedForRequest(RelapseRequest):
    """Request object which honours proxy headers

    Extends RelapseRequest to replace getClientIP, getClientAddress, and isSecure with
...
class RelapseSite(ProxySite):
    ...
    First, it replaces the requestFactory in use so that we build RelapseRequests
    instead of regular t.w.server.Requests. All of the  constructor params are really
    just parameters for RelapseRequest.

    Second, it inhibits the log() method called by Request.finish, since RelapseRequest
    ...
    def __init__(
        ...
        request_class = XForwardedForRequest if proxied else RelapseRequest
