...
from relapse.util.async_helpers import AwakenableSleeper, timeout_deferred
...
outgoing_requests_counter = Counter(
    "relapse_http_matrixfederationclient_requests", "", ["method"]
)
incoming_responses_counter = Counter(
    "relapse_http_matrixfederationclient_responses", "", ["method", "code"]
)
...
class MatrixFederationHttpClient:
    ...
    async def _send_request_with_optional_trailing_slash(
        ...
        **send_request_args: Any,
        ...
        'M_UNRECOGNIZED' errcode. This is a workaround for Relapse <= v0.99.3
        ...
        try:
            ...
            if e.code != 400 or e.to_relapse_error().errcode != "M_UNRECOGNIZED":
                raise

            # Retry with a trailing slash if we received a 400 with
            # 'M_UNRECOGNIZED' which some endpoints can return when omitting a
            # trailing slash on Relapse <= v0.99.3.
    ...
    async def _send_request(
        ...
        limiter = await relapse.util.retryutils.get_retry_limiter(
            request.destination,
            self.clock,
            self._store,
    ...
    async def put_json(
        ...
        Args:
            ...
            try_trailing_slash_on_400: True if on a 400 M_UNRECOGNIZED
                ...
                in Relapse <= v0.99.3. This will be attempted before backing off if
    ...
    async def get_file(
        ...
        try:
            ...
            raise RelapseError(HTTPStatus.BAD_GATEWAY, msg, Codes.TOO_LARGE)
        except defer.TimeoutError as e:
            logger.warning(
                "{%s} [%s] Timed out reading response - %s %s",
                request.txn_id,
