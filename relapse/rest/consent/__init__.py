...
from relapse.api.errors import NotFoundError, RelapseError, StoreError
from relapse.config import ConfigError
from relapse.http.server import respond_with_html
from relapse.http.servlet import RestServlet, parse_bytes_from_args, parse_string
...
class ConsentServlet(RestServlet):
    """A servlet to display a privacy policy and gather consent to it
    ...
    PATTERNS = [re.compile(r"/_matrix/consent$")]
    ...
    def _check_hash(self, userid: str, userhmac: bytes) -> None:
        ...
        Raises:
              RelapseError if the hash doesn't match
