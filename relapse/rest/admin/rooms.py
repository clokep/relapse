...
from relapse.api.constants import Direction, EventTypes, JoinRules, Membership
...
from relapse.storage.databases.main.room import RoomSortOrder
from relapse.streams.config import PaginationConfig
from relapse.types import JsonDict, RoomID, ScheduledTask, UserID, create_requester
...
if TYPE_CHECKING:
    ...
    from relapse.handlers.room import RoomShutdownHandler
...
class MakeRoomAdminRestServlet(ResolveRoomIdMixin, RestServlet):
    ...
    async def on_POST(
        ...
        if not filtered_room_state:
            raise RelapseError(HTTPStatus.BAD_REQUEST, "Server not in room")
...
class RoomTimestampToEventRestServlet(RestServlet):
    ...
    GET /_relapse/admin/v1/rooms/<roomID>/timestamp_to_event?ts=<timestamp>&dir=<direction>
    {
        "event_id": ...
    }
    """
