...
from relapse.types import EventID, JsonDict, RoomID
...
class ReceiptRestServlet(RestServlet):
    ...
    async def on_POST(
        self, request: RelapseRequest, room_id: str, receipt_type: str, event_id: str
    ) -> Tuple[int, JsonDict]:
        requester = await self.auth.get_user_by_req(request)

        if not RoomID.is_valid(room_id) or not event_id.startswith(EventID.SIGIL):
            raise RelapseError(400, "A valid room ID and event ID must be specified")
