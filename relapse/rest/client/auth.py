...
from relapse.api.errors import LoginError, RelapseError
...
class AuthRestServlet(RestServlet):
    ...
    async def on_GET(self, request: RelapseRequest, stagetype: str) -> None:
        session = parse_string(request, "session")
        if not session:
            raise RelapseError(400, "No session supplied")
        ...
        if stagetype == LoginType.RECAPTCHA:
            ...
            raise RelapseError(404, "Unknown auth stage type")
    ...
    async def on_POST(self, request: Request, stagetype: str) -> None:
        ...
        if stagetype == LoginType.RECAPTCHA:
            ...
            if not response:
                raise RelapseError(400, "No captcha response supplied")
            ...
            raise RelapseError(404, "Fallback SSO auth does not support POST requests.")
