...
from relapse.metrics import threepid_send_requests
from relapse.push.mailer import Mailer
from relapse.rest.client.models import (
    AuthenticationData,
    ClientSecretStr,
...
from relapse.util.stringutils import assert_valid_client_secret, random_string
from relapse.util.threepids import check_3pid_allowed, validate_email
...
class EmailPasswordRequestTokenRestServlet(RestServlet):
    ...
    async def on_POST(self, request: RelapseRequest) -> tuple[int, JsonDict]:
        ...
        if existing_user_id is None:
            ...
            raise RelapseError(400, "Email not found", Codes.THREEPID_NOT_FOUND)

        # Send password reset emails from Relapse
...
class PasswordRestServlet(RestServlet):
    ...
    async def on_POST(self, request: RelapseRequest) -> tuple[int, JsonDict]:
            ...
            if self.auth.has_access_token(request):
                ...
                if LoginType.EMAIL_IDENTITY in result:
                    ...
                    if "medium" not in threepid or "address" not in threepid:
                        raise RelapseError(500, "Malformed threepid")
                    ...
                    raise RelapseError(500, "", Codes.UNKNOWN)
        ...
        if not password_hash:
            raise RelapseError(400, "Missing params: password", Codes.MISSING_PARAM)
...
class EmailThreepidRequestTokenRestServlet(RestServlet):
    ...
    async def on_POST(self, request: RelapseRequest) -> tuple[int, JsonDict]:
        ...
        if existing_user_id is not None:
            ...
            raise RelapseError(400, "Email is already in use", Codes.THREEPID_IN_USE)

        # Send threepid validation emails from Relapse
        sid = await self.identity_handler.send_threepid_validation(
            body.email,
            body.client_secret,
            body.send_attempt,
...
class MsisdnThreepidRequestTokenRestServlet(RestServlet):
    ...
    async def on_POST(self, request: RelapseRequest) -> tuple[int, JsonDict]:
        ...
        if existing_user_id is not None:
            ...
            raise RelapseError(400, "MSISDN is already in use", Codes.THREEPID_IN_USE)
...
class ThreepidDeleteRestServlet(RestServlet):
    ...
    async def on_POST(self, request: RelapseRequest) -> tuple[int, JsonDict]:
        ...
        try:
            ...
            raise RelapseError(500, "Failed to remove threepid")
...
def assert_valid_next_link(hs: "HomeServer", next_link: str) -> None:
    """
    Raises a RelapseError if a given next_link value is invalid
    ...
    Raises:
        RelapseError: If the next_link is invalid
    """
    valid = True
