...
from relapse.util.threepids import (
    canonicalise_email,
    check_3pid_allowed,
    validate_email,
)
...
class EmailRegisterRequestTokenRestServlet(RestServlet):
    ...
    async def on_POST(self, request: RelapseRequest) -> tuple[int, JsonDict]:
        ...
        # in relapse/rest/client/account.py)
        try:
        ...
        # Send registration emails from Relapse
...
class RegisterRestServlet(RestServlet):
    ...
    async def on_POST(self, request: RelapseRequest) -> tuple[int, JsonDict]:
        ...
        if not isinstance(client_requested_refresh_tokens, bool):
            raise RelapseError(400, "`refresh_token` should be true or false.")
        ...
        if "username" in body:
            ...
            if not isinstance(desired_username, str) or len(desired_username) > 512:
                raise RelapseError(400, "Invalid username")
        ...
        if body.get("type") == APP_SERVICE_REGISTRATION_TYPE:
            ...
            if desired_username is None:
                raise RelapseError(400, "Desired Username is missing or not a string")
        ...
        if not self._registration_enabled:
            raise RelapseError(403, "Registration has been disabled", Codes.FORBIDDEN)
        ...
        if password is not None:
            if not isinstance(password, str) or len(password) > 512:
                raise RelapseError(400, "Invalid password")
    ...
    async def _do_guest_registration(
        ...
        device_id = relapse.api.auth.GUEST_DEVICE_ID
        initial_display_name = params.get("initial_device_display_name")
        (
            device_id,
            access_token,
...
class RegisterAppServiceOnlyRestServlet(RestServlet):
    ...
    async def on_POST(self, request: RelapseRequest) -> tuple[int, JsonDict]:
        ...
        if body.get("type") != APP_SERVICE_REGISTRATION_TYPE:
            raise RelapseError(403, "Non-application service registration type")
