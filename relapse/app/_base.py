...
import relapse.util.caches
from relapse.api.constants import MAX_PDU_SIZE
from relapse.app import check_bind_error
from relapse.app.phone_stats_home import start_phone_stats_home
from relapse.config import ConfigError
from relapse.config._base import format_config_error
...
from relapse.logging.opentracing import init_tracer
...
from relapse.types import IRelapseReactor, StrCollection
from relapse.util import RELAPSE_VERSION
from relapse.util.caches.lrucache import setup_expire_lru_cache_entries
from relapse.util.daemonize import daemonize_process
from relapse.util.gai_resolver import GAIResolver
from relapse.util.rlimit import change_resource_limit
...
def start_worker_reactor(
    ...
    # (needed when relapse.app.complement_fork_starter is being used)
...
def listen_http(
    ...
    if isinstance(listener_config, TCPListenerConfig):
        if listener_config.is_tls():
            ...
            logger.info(
                "Relapse now listening on TCP port %d (TLS)", listener_config.port
            )
        else:
            ports = listen_tcp(
                listener_config.bind_addresses,
        ...
        logger.info(
            "Relapse now listening on Unix Socket at: "
...
def refresh_certificate(hs: "HomeServer") -> None:
    """
    Refresh the TLS certificates that Relapse is using by re-reading them from
...
async def start(hs: "HomeServer") -> None:
    """
    Start a Relapse server or worker.
    ...
    # It is now safe to start your Relapse.
    hs.start_listening()
...
def reload_cache_config(config: HomeServerConfig) -> None:
    ...
    Note that the following cache config keys are read, but not applied:
        ...
        - track_memory_usage. This affects relapse.util.caches.TRACK_MEMORY_USAGE which
              influences Relapse's self-reported metrics.
