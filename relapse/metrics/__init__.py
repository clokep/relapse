...
sent_transactions_counter = Counter("relapse_federation_client_sent_transactions", "")

events_processed_counter = Counter("relapse_federation_client_events_processed", "")

event_processing_loop_counter = Counter(
    "relapse_event_processing_loop_count", "Event processing loop iterations", ["name"]
)

event_processing_loop_room_count = Counter(
    "relapse_event_processing_loop_room_count",
...
event_processing_positions = Gauge("relapse_event_processing_positions", "", ["name"])

# Used to track the current max events stream position
event_persisted_position = Gauge("relapse_event_persisted_position", "")

# Used to track the received_ts of the last event processed by various
# components
event_processing_last_ts = Gauge("relapse_event_processing_last_ts", "", ["name"])

# Used to track the lag processing events. This is the time difference
# between the last processed event's received_ts and the time it was
# finished being processed.
event_processing_lag = Gauge("relapse_event_processing_lag", "", ["name"])

event_processing_lag_by_event = Histogram(
    "relapse_event_processing_lag_by_event",
...
build_info = Gauge(
    "relapse_build_info", "Build information", ["pythonversion", "version", "osversion"]
)
build_info.labels(
...
threepid_send_requests = Histogram(
    "relapse_threepid_send_requests_with_tries",
