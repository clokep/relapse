...
from relapse.logging.opentracing import RelapseTags, start_active_span
...
_background_process_start_count = Counter(
    "relapse_background_process_start_count",
    "Number of background processes started",
    ["name"],
)

_background_process_in_flight_count = Gauge(
    "relapse_background_process_in_flight_count",
...
_background_process_db_txn_duration = Counter(
    "relapse_background_process_db_txn_duration_seconds",
    (
...
_background_process_db_sched_duration = Counter(
    "relapse_background_process_db_sched_duration_seconds",
...
def run_as_background_process(
    ...
    **kwargs: Any,
    ...
    follow the relapse logcontext rules, which makes it appropriate for passing to
    clock.looping_call and friends (or for firing-and-forgetting in the middle of a
    normal relapse async function).
    ...
    Returns:
        ...
        Note that the returned Deferred does not follow the relapse logcontext
        rules.
    """
            ...
            try:
                if bg_start_span:
                    ctx = start_active_span(
                        f"bgproc.{desc}", tags={RelapseTags.REQUEST_ID: str(context)}
                    )
                else:
