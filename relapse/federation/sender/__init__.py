...
*Catch-Up Mode*, entered upon Relapse startup or once a homeserver has fallen behind due to
...
This reduces load on the destination **in aggregate** because all Relapse homeservers
...
from relapse.federation.sender.per_destination_queue import (
    CATCHUP_RETRY_INTERVAL,
    PerDestinationQueue,
)
from relapse.federation.sender.transaction_manager import TransactionManager
from relapse.federation.units import Edu
from relapse.logging.context import make_deferred_yieldable, run_in_background
from relapse.metrics import (
    LaterGauge,
...
from relapse.types import JsonDict, ReadReceipt, RoomStreamToken, StrCollection
from relapse.util import Clock
from relapse.util.metrics import Measure
from relapse.util.retryutils import filter_destinations_by_retry_limiter
...
sent_pdus_destination_dist_count = Counter(
    "relapse_federation_client_sent_pdu_destinations_count",
    "Number of PDUs queued for sending to one or more destinations",
)

sent_pdus_destination_dist_total = Counter(
    "relapse_federation_client_sent_pdu_destinations",
...
class FederationSender(AbstractFederationSender):
    def __init__(self, hs: "HomeServer"):
        ...
        LaterGauge(
            "relapse_federation_transaction_queue_pending_pdus",
            "",
            [],
            lambda: sum(
        ...
        LaterGauge(
            "relapse_federation_transaction_queue_pending_edus",
            "",
            [],
            lambda: sum(
            ...
            while True:
                ...
                async def handle_event(event: EventBase) -> None:
                    ...
                    if sharded_destinations:
                        ...
                        relapse.metrics.event_processing_lag_by_event.labels(
                            "federation_sender"
                        ).observe((now - ts) / 1000)
                ...
                if event_entries:
                    ...
                    relapse.metrics.event_processing_lag.labels(
                        "federation_sender"
                    ).set(now - ts)
                    relapse.metrics.event_processing_last_ts.labels(
                        "federation_sender"
                    ).set(ts)
                ...
                relapse.metrics.event_processing_positions.labels(
                    "federation_sender"
                ).set(next_token)
