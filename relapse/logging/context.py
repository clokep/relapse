...
if TYPE_CHECKING:
    from relapse.logging.scopecontextmanager import _LogContextScope
...
class ContextRequest:
    """
    A bundle of attributes from the RelapseRequest object.

    This exists to:

    * Avoid a cycle between LoggingContext and RelapseRequest.
...
class LoggingContext:
    ...
    def current_context(cls) -> LoggingContextOrSentinel:
        ...
        """
        warnings.warn(
            "relapse.logging.context.LoggingContext.current_context() is deprecated "
            "in favor of relapse.logging.context.current_context().",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def set_current_context(
        ...
        """
        warnings.warn(
            "relapse.logging.context.LoggingContext.set_current_context() is deprecated "
            "in favor of relapse.logging.context.set_current_context().",
            DeprecationWarning,
            stacklevel=2,
        )
...
def make_deferred_yieldable(deferred: "defer.Deferred[T]") -> "defer.Deferred[T]":
    """Given a deferred, make it follow the Relapse logcontext rules:
...
def defer_to_thread(
    reactor: "IRelapseReactor", f: Callable[P, R], *args: P.args, **kwargs: P.kwargs
) -> "defer.Deferred[R]":
    """
    ...
    The result deferred follows the Relapse logcontext rules: you should `yield`
    on it.
