...
"""
============================
Using OpenTracing in Relapse
============================

Python-specific tracing concepts are at https://opentracing.io/guides/python/.
Note that Relapse wraps OpenTracing in a small module (this one) in order to make the
...
no span objects from OpenTracing are exposed in Relapse's code. This allows
OpenTracing to be easily disabled in Relapse and thereby have OpenTracing as
...
in the code snippets refer to the Relapses module.
...
In Relapse it is not possible to start a non-active span. Spans can be started
...
.. code-block:: python
   ...
   from relapse.logging.opentracing import start_active_span
...
.. code-block:: python
   ...
   from relapse.logging.opentracing import trace_with_opname
...
.. code-block:: python
   ...
   from relapse.logging.opentracing import tag_args
...
class RelapseBaggage:
    FORCE_TRACING = "relapse-force-tracing"
...
def init_tracer(hs: "HomeServer") -> None:
    ...
    # tag the Relapse instance name so that it's an easy jumping
    # off point into the logs. Can also be used to filter for an
    # instance that is under load.
    tags[RelapseTags.INSTANCE_NAME] = hs.get_instance_name()
...
def force_tracing(
    ...
    # it is enabled later
    span_to_trace.set_baggage_item(RelapseBaggage.FORCE_TRACING, "1")
...
def is_context_forced_tracing(
    ...
    if span_context is None:
        return False
    return span_context.baggage.get(RelapseBaggage.FORCE_TRACING) is not None
...
def inject_header_dict(
    ...
    Note:
        ...
        relapse or twisted. If we're still using jaeger these headers would be those
        here:
...
def inject_response_headers(response_headers: Headers) -> None:
    ...
    if trace_id is not None:
        response_headers.addRawHeader("Relapse-Trace-Id", f"{trace_id:x}")
...
def tag_args(func: Callable[P, R]) -> Callable[P, R]:
    ...
    def _wrapping_logic(
        ...
        for i, arg in enumerate(args[1:], start=1):
            set_tag(RelapseTags.FUNC_ARG_PREFIX + argspec.args[i], str(arg))
        set_tag(RelapseTags.FUNC_ARGS, str(args[len(argspec.args) :]))
        set_tag(RelapseTags.FUNC_KWARGS, str(kwargs))
        yield
...
@contextlib.contextmanager
def trace_servlet(
    request: "RelapseRequest", extract_context: bool = False
) -> Generator[None, None, None]:
    ...
    request_tags = {
        RelapseTags.REQUEST_ID: request.get_request_id(),
    ...
    # pass the span into the RelapseRequest, which will finish it once we've finished
