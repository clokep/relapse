...
class EventClientSerializer:
    ...
    async def serialize_events(
        self,
        events: Iterable[JsonDict | EventBase],
        time_now: int,
        *,
        config: SerializeEventConfig = _DEFAULT_SERIALIZE_EVENT_CONFIG,
        bundle_aggregations: dict[str, "BundledAggregations"] | None = None,
    ...
    * NaN, Infinity, -Infinity
    ...
    if type(value) is int:  # noqa: E721
        if value < CANONICALJSON_MIN_INT or CANONICALJSON_MAX_INT < value:
            raise RelapseError(400, "JSON integer out of range", Codes.BAD_JSON)

    elif isinstance(value, float):
        # Note that Infinity, -Infinity, and NaN are also considered floats.
        raise RelapseError(400, "Bad JSON value: float", Codes.BAD_JSON)
        ...
        raise RelapseError(400, "Unknown JSON value", Codes.BAD_JSON)
