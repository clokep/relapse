...
from relapse.storage.database import LoggingDatabaseConnection, LoggingTransaction
from relapse.storage.engines import BaseDatabaseEngine, PostgresEngine, Sqlite3Engine
from relapse.storage.schema import SCHEMA_COMPAT_VERSION, SCHEMA_VERSION
...
OUTDATED_SCHEMA_ON_WORKER_ERROR = (
    "Expected database schema version %i but got %i: run the main relapse process to "
    "upgrade the database schema before starting worker processes."
)

EMPTY_DATABASE_ON_WORKER_ERROR = (
    "Uninitialised database: run the main relapse process to prepare the database "
    "schema before starting worker processes."
)

UNAPPLIED_DELTA_ON_WORKER_ERROR = (
    "Database schema delta %s has not been applied: run the main relapse process to "
    "upgrade the database schema before starting worker processes."
)
...
def _upgrade_existing_database(
    ...
    There can be multiple delta files per version. Relapse will keep track of
    ...
    if not is_empty and "main" in databases:
        from relapse.storage.databases.main import check_database_before_upgrade

        assert config is not None
        check_database_before_upgrade(cur, database_engine, config)

    # update schema_compat_version before we run any upgrades, so that if relapse
