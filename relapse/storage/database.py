...
from relapse.metrics import LaterGauge, register_threadpool
...
from relapse.storage.types import Connection, Cursor, SQLQueryParameters
...
sql_logger = logging.getLogger("relapse.storage.SQL")
transaction_logger = logging.getLogger("relapse.storage.txn")
perf_logger = logging.getLogger("relapse.storage.TIME")

sql_scheduling_timer = Histogram("relapse_storage_schedule_time", "sec")

sql_query_timer = Histogram("relapse_storage_query_time", "sec", ["verb"])
sql_txn_count = Counter("relapse_storage_transaction_time_count", "sec", ["desc"])
sql_txn_duration = Counter("relapse_storage_transaction_time_sum", "sec", ["desc"])
...
class DatabasePool:
    ...
    def __init__(
        ...
        LaterGauge(
            "relapse_background_update_status",
            ...
            while True:
                ...
                try:
                    with opentracing.start_active_span(
                        "db.txn",
                        tags={
                            opentracing.RelapseTags.DB_TXN_DESC: desc,
                            opentracing.RelapseTags.DB_TXN_ID: name,
                        },
                    ):
