...
from relapse.replication.tcp.streams import ToDeviceStream
        ...
        def get_device_messages_txn(
            ...
            for row in txn:
                ...
                with start_active_span("get_to_device_message"):
                    set_tag(RelapseTags.TO_DEVICE_TYPE, message_dict["type"])
                    set_tag(RelapseTags.TO_DEVICE_SENDER, message_dict["sender"])
                    set_tag(RelapseTags.TO_DEVICE_RECIPIENT, recipient_user_id)
                    set_tag(RelapseTags.TO_DEVICE_RECIPIENT_DEVICE, recipient_device_id)
                    set_tag(
                        RelapseTags.TO_DEVICE_MSGID,
                ...
                for user_id, messages_by_device in edu["messages"].items():
                    for device_id, msg in messages_by_device.items():
                        with start_active_span("store_outgoing_to_device_message"):
                            set_tag(RelapseTags.TO_DEVICE_EDU_ID, edu["sender"])
                            set_tag(RelapseTags.TO_DEVICE_EDU_ID, edu["message_id"])
                            set_tag(RelapseTags.TO_DEVICE_TYPE, edu["type"])
                            set_tag(RelapseTags.TO_DEVICE_RECIPIENT, user_id)
                            set_tag(RelapseTags.TO_DEVICE_RECIPIENT_DEVICE, device_id)
                            set_tag(
                                RelapseTags.TO_DEVICE_MSGID,
            ...
            if len(devices) == 1 and devices[0] == "*":
                ...
                for (device_id,) in rows:
                    ...
                    with start_active_span("serialise_to_device_message"):
                        ...
                        set_tag(RelapseTags.TO_DEVICE_TYPE, msg["type"])
                        set_tag(RelapseTags.TO_DEVICE_SENDER, msg["sender"])
                        set_tag(RelapseTags.TO_DEVICE_RECIPIENT, user_id)
                        set_tag(RelapseTags.TO_DEVICE_RECIPIENT_DEVICE, device_id)
                        set_tag(
                            RelapseTags.TO_DEVICE_MSGID,
