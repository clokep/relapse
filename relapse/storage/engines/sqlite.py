...
class Sqlite3Engine(BaseDatabaseEngine[sqlite3.Connection, sqlite3.Cursor]):
    ...
    def check_database(
        ...
        if not allow_outdated_version:
            # Relapse is untested against older SQLite versions, and we don't want
            # to let users upgrade to a version of Relapse with broken support for their
            # sqlite version, because it risks leaving them with a half-upgraded db.
            if sqlite3.sqlite_version_info < (3, 27, 0):
                raise RuntimeError("Relapse requires sqlite 3.27 or above.")
