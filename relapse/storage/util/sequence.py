...
table '%(table)s'. This can happen if Relapse has been downgraded and
...
A temporary workaround to fix this error is to shut down Relapse (including
...
class SequenceGenerator(metaclass=abc.ABCMeta):
    ...
    def check_consistency(
        ...
        sync with the table, e.g. if Relapse gets rolled back to a previous
...
class LocalSequenceGenerator(SequenceGenerator):
    ...
    def get_next_id_txn(self, txn: Cursor) -> int:
        ...
        # we are a single process relapse.
        with self._lock:
