...
class RelapseError(CodeMessageException):
    ...
    probably shouldn't be a `RelapseError`. For example, if we contact another
...
class InvalidAPICallError(RelapseError):
...
class ConsentNotGivenError(RelapseError):
...
class UserDeactivatedError(RelapseError):
...
class FederationDeniedError(RelapseError):
...
class UnrecognizedRequestError(RelapseError):
...
class NotFoundError(RelapseError):
...
class OAuthInsufficientScopeError(RelapseError):
...
class ResourceLimitError(RelapseError):
    """
...
class EventSizeError(RelapseError):
...
class RoomKeysVersionError(RelapseError):
...
class UnsupportedRoomVersionError(RelapseError):
...
class ThreepidValidationError(RelapseError):
...
class IncompatibleRoomVersionError(RelapseError):
...
class PasswordRefusedError(RelapseError):
...
class NotApprovedError(RelapseError):
    def __init__(
        self,
        msg: str,
...
class HttpResponseException(CodeMessageException):
    ...
    def to_relapse_error(self) -> RelapseError:
        """Make a RelapseError based on an HTTPResponseException
        ...
        are used as the errcode and error message in the new relapse error.

        Otherwise, the errcode is set to M_UNKNOWN, and the error message is
        set to the reason code from the HTTP response.

        Returns:
            The error converted to a RelapseError.
        """
...
class PartialStateConflictError(RelapseError):
