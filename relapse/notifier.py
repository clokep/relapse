...
from relapse.api.constants import EduTypes, EventTypes, HistoryVisibility, Membership
...
from relapse.logging.opentracing import log_kv, start_active_span
...
from relapse.util.async_helpers import ObservableDeferred, timeout_deferred
...
notified_events_counter = Counter("relapse_notifier_notified_events", "")

users_woken_by_stream_counter = Counter(
    "relapse_notifier_users_woken_by_stream", "", ["stream"]
)
...
class Notifier:
    ...
    def __init__(self, hs: "HomeServer"):
        ...
        LaterGauge("relapse_notifier_listeners", "", [], count_listeners)

        LaterGauge(
            "relapse_notifier_rooms",
            "",
            [],
            lambda: count(bool, list(self.room_to_user_streams.values())),
        )
        LaterGauge(
            "relapse_notifier_users", "", [], lambda: len(self.user_to_user_stream)
        )
