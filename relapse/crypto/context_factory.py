...
def _context_info_cb(ssl_connection: SSL.Connection, where: int, ret: int) -> None:
    ...
    try:
        ...
        # '_relapse_tls_verifier' attribute to a ConnectionVerifier object.
        tls_protocol._relapse_tls_verifier.verify_context_info_cb(ssl_connection, where)
...
class SSLClientConnectionCreator:
    ...
    def clientConnectionForTLS(
        ...
        # ... and we also gut-wrench a '_relapse_tls_verifier' attribute into the
        # tls_protocol so that the SSL context's info callback has something to
        # call to do the cert verification.
        tls_protocol._relapse_tls_verifier = self._verifier  # type: ignore[attr-defined]
        return connection
