...
def validate_event_for_room_version(event: "EventBase") -> None:
    ...
    Raises:
        RelapseError if there is a problem with the event
    """
...
# Set of room versions where Relapse did not apply event key size limits
...
def _check_size_limits(event: "EventBase") -> None:
    ...
    The codepoint size limits are only for Relapse compatibility.

    Raises:
        EventSizeError:
            when a size limit has been violated.

            unpersistable=True if Relapse never would have accepted the event and
                the PDU must NOT be persisted.

            unpersistable=False if a prior version of Relapse would have accepted the
    ...
    # Codepoint size check: Relapse always enforced these limits, so apply
    # them strictly.
