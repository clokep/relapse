...
from relapse.api.room_versions import KNOWN_ROOM_VERSIONS, StateResolutionVersions
from relapse.events import EventBase
from relapse.events.snapshot import (
    EventContext,
...
from relapse.replication.http.state import ReplicationUpdateCurrentStateRestServlet
from relapse.state import v1, v2
from relapse.storage.databases.main.events_worker import EventRedactBehaviour
from relapse.types import StateMap, StrCollection
...
from relapse.util.metrics import Measure, measure_func

if TYPE_CHECKING:
    from relapse.server import HomeServer
    from relapse.storage.controllers import StateStorageController
    from relapse.storage.databases.main import DataStore

logger = logging.getLogger(__name__)
metrics_logger = logging.getLogger("relapse.state.metrics")

# Metrics for number of state groups involved in a resolution.
state_groups_histogram = Histogram(
    "relapse_state_number_state_groups_in_resolution",
...
class StateHandler:
    ...
    async def update_current_state(self, room_id: str) -> None:
        ...
        Raises:
            RelapseError(502): if all attempts to connect to the event persister worker
                fail
        """
...
_biggest_room_by_cpu_counter = Counter(
    "relapse_state_res_cpu_for_biggest_room_seconds",
...
_biggest_room_by_db_counter = Counter(
    "relapse_state_res_db_for_biggest_room_seconds",
...
_cpu_times = Histogram(
    "relapse_state_res_cpu_for_all_rooms_seconds",
    "CPU time (utime+stime) spent computing a single state resolution",
)
_db_times = Histogram(
    "relapse_state_res_db_for_all_rooms_seconds",
