...
from relapse.types import MutableStateMap, StateMap, StrCollection
...
class StateResolutionStore(Protocol):
    # This is usually relapse.state.StateResolutionStore, but it's replaced with a
...
async def _iterative_auth_checks(
    ...
    for idx, event_id in enumerate(event_ids, start=1):
        ...
        if event.rejected_reason is not None:
            ...
            #       state resolution should be accepted. Relapse does not handle the
