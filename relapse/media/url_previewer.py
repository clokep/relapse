...
from relapse.media._base import FileInfo, get_filename_from_headers
from relapse.media.media_storage import MediaStorage
from relapse.media.oembed import OEmbedProvider
from relapse.media.preview_html import decode_body, parse_html_to_open_graph
...
class UrlPreviewer:
    ...
    When Relapse is asked to preview a URL it does the following:
    ...
    async def _do_preview(self, url: str, user: UserID, ts: int) -> bytes:
        ...
        if _is_media(media_info.media_type):
            ...
            # TODO: somehow stop a big HTML document from exploding relapse's RAM
        ...
        try:
            ...
            length, headers, uri, code = await self.client.get_file(
                ...
                headers={
                    ...
                    # Open Graph metadata to crawler user agents. Omit the Relapse version
                    # string to avoid leaking information.
                    b"User-Agent": [
                        "Relapse (bot; +https://github.com/clokep/relapse)"
                    ],
                },
                is_allowed_content_type=_is_previewable,
            )
        except RelapseError:
            # Pass RelapseErrors through directly, so that the servlet
            # handler will return a RelapseError to the client instead of
            # blank data or a 500.
            raise
    ...
    async def _handle_url(
        ...
        Raises:
            RelapseError if the URL is blocked.
        """
