...
from relapse.push import Pusher, PusherConfig, PusherConfigException
from relapse.push.pusher import PusherFactory
from relapse.replication.http.push import ReplicationRemovePusherRestServlet
from relapse.types import JsonDict, RoomStreamToken, StrCollection
...
relapse_pushers = Gauge(
    "relapse_pushers", "Number of active relapse pushers", ["kind", "app_id"]
)
...
class PusherPool:
    ...
    async def _start_pusher(self, pusher_config: PusherConfig) -> Optional[Pusher]:
        ...
        if appid_pushkey in byuser:
            ...
            relapse_pushers.labels(
                type(previous_pusher).__name__, previous_pusher.app_id
            ).dec()
        byuser[appid_pushkey] = pusher

        relapse_pushers.labels(type(pusher).__name__, pusher.app_id).inc()
    ...
    def maybe_stop_pusher(self, app_id: str, pushkey: str, user_id: str) -> None:
        ...
        if appid_pushkey in byuser:
            ...
            relapse_pushers.labels(type(pusher).__name__, pusher.app_id).dec()
