...
class TaskScheduler:
    ...
    The problem with that is that the tasks will just stop and never be resumed if relapse
    is stopped for whatever reason.

    How this works:
    - A function mapped to a named action should first be registered with `register_action`.
    This function will be called when trying to resuming tasks after a relapse shutdown,
    so this registration should happen when relapse is initialised, NOT right before scheduling
    a task.
    ...
    def __init__(self, hs: "HomeServer"):
        ...
        LaterGauge(
            "relapse_scheduler_running_tasks",
    ...
    async def update_task(
        ...
        *,
        ...
        resume a task at a specific step after a restart of relapse.
