...
import relapse
from relapse.relapse_rust import get_rust_file_digest
...
def check_rust_lib_up_to_date() -> None:
    ...
    relapse_dir = os.path.dirname(relapse.__file__)
    relapse_root = os.path.abspath(os.path.join(relapse_dir, ".."))

    # Double check we've not gone into site-packages...
    if os.path.basename(relapse_root) == "site-packages":
        return
    ...
    hash = _hash_rust_files_in_directory(os.path.join(relapse_root, "rust", "src"))
...
def _dist_is_editable() -> bool:
    ...
    for path_item in sys.path:
        egg_link = os.path.join(path_item, "matrix-relapse.egg-link")
