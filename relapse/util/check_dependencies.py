...
"""
This module exposes a single function which checks relapse's dependencies are present
...
DISTRIBUTION_NAME = "matrix-relapse"
...
def _not_installed(requirement: Requirement, extra: Optional[str] = None) -> str:
    if extra:
        ...
        return f"Relapse {VERSION} needs {requirement.name}, but it is not installed"
...
def _incorrect_version(
    ...
    if extra:
        ...
        return (
            f"Relapse {VERSION} needs {requirement}, but got {requirement.name}=={got}"
        )
...
def _no_reported_version(requirement: Requirement, extra: Optional[str] = None) -> str:
    if extra:
        ...
        return (
            f"Relapse {VERSION} needs {requirement}, "
            f"but can't determine {requirement.name}'s version"
        )
...
def check_requirements(extra: Optional[str] = None) -> None:
    ...
    `pip install matrix-relapse[saml2]`).
    ...
    if extra is None:
        ...
        raise ValueError(f"Relapse {VERSION} does not provide the feature '{extra}'")
