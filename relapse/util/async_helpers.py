...
class AbstractObservableDeferred(Generic[_T], metaclass=abc.ABCMeta):
    ...
    def observe(self) -> "defer.Deferred[_T]":
        ...
        Note that the returned Deferred doesn't follow the Relapse logcontext rules -
...
def delay_cancellation(awaitable: Awaitable[T]) -> Awaitable[T]:
    ...
    Args:
        deferred: The coroutine or `Deferred` to protect against cancellation. May
            optionally follow the Relapse logcontext rules.
    ...
    Returns:
        ...
        The new `Deferred` will follow the Relapse logcontext rules if `awaitable`
        follows the Relapse logcontext rules. Otherwise the new `Deferred` should be
        wrapped with `make_deferred_yieldable`.
    """
