...
from relapse.events.utils import SerializeEventConfig, maybe_upsert_event_field
...
from relapse.replication.http.send_event import ReplicationSendEventRestServlet
from relapse.replication.http.send_events import ReplicationSendEventsRestServlet
...
from relapse.util import json_decoder, json_encoder, log_failure, unwrapFirstError
from relapse.util.async_helpers import Linearizer, gather_results
from relapse.util.caches.expiringcache import ExpiringCache
from relapse.util.metrics import measure_func
from relapse.visibility import get_effective_room_visibility_from_state
...
class MessageHandler:
    ...
    async def get_room_data(
        ...
        Raises:
            RelapseError or AuthError if the user is not in the room
        """
        (
            membership,
            membership_event_id,
    ...
    async def get_state_events(
        ...
        Args:
            ...
            at_token: the stream token of the at which we are requesting
                ...
                stream token, we raise a 403 RelapseError. If None, returns the current
...
class EventCreationHandler:
    ...
    async def create_event(
        ...
        # See `relapse.events.utils.EventClientSerializer.serialize_event`
        ...
        # relapse.handlers.directory pointless.
        ...
        for i in range(max_retries):
            try:
                ...
                if spam_check_result != self._spam_checker_module_callbacks.NOT_SPAM:
                    ...
                    # RelapseError (which is now deprecated).
                    raise RelapseError(
                        403,
                        spam_check_result,
                        Codes.FORBIDDEN,
    ...
    async def _validate_event_relation(self, event: EventBase) -> None:
        ...
        Raises:
            RelapseError if the event is invalid.
        """
        ...
        if parent_event:
            ...
            if parent_event.room_id != event.room_id:
                raise RelapseError(400, "Relations must be in the same room")
            ...
            if not await self.store.event_is_target_of_relation(relation.parent_id):
                ...
                raise RelapseError(400, "Can't send relation to unknown event")
        ...
        if relation.rel_type == RelationTypes.ANNOTATION:
            ...
            if aggregation_key is None:
                raise RelapseError(400, "Missing aggregation key")

            if len(aggregation_key) > 500:
                raise RelapseError(400, "Aggregation key is too long")
