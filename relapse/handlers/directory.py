...
from relapse.storage.databases.main.directory import RoomAliasMapping
from relapse.types import JsonDict, Requester, RoomAlias
...
class DirectoryHandler:
    ...
    async def _create_association(
        ...
        if ":" in room_alias.localpart:
            raise RelapseError(400, "Invalid character in room alias localpart: ':'.")

        if not self.hs.is_mine(room_alias):
            raise RelapseError(400, "Room alias must be local")
        ...
        if not servers:
            raise RelapseError(400, "Failed to get server list")
    ...
    async def create_association(
        ...
        if service:
            ...
            if not self.config.roomdirectory.is_alias_creation_allowed(
                ...
                raise RelapseError(403, "Not allowed to create alias")
    ...
    async def delete_association(
        ...
        Raises:
            ...
            RelapseError: if the alias belongs to an AS
        """
        user_id = requester.user.to_string()
    ...
    async def get_association(self, room_alias: RoomAlias) -> JsonDict:
        ...
        if self.hs.is_mine(room_alias):
            ...
            try:
                ...
                raise RelapseError(502, "Failed to fetch alias")
    ...
    async def on_directory_query(self, args: JsonDict) -> JsonDict:
        ...
        if not self.hs.is_mine(room_alias):
            raise RelapseError(400, "Room Alias is not hosted on this homeserver")
    ...
    async def edit_published_room_list(
        ...
        if room is None:
            raise RelapseError(400, "Unknown room")
