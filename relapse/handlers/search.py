...
from relapse.types import JsonDict, Requester, StrCollection, StreamKeyType, UserID
...
class SearchHandler:
    ...
    async def search(
        ...
        if not self.hs.config.server.enable_search:
            raise RelapseError(400, "Search is disabled on this homeserver")
        ...
        if batch:
            try:
                ...
                raise RelapseError(400, "Invalid batch")
    ...
    async def _search_by_recent(
        ...
        # But only go around 5 times since otherwise relapse will be sad.
