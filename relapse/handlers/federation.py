...
from relapse.crypto.event_signing import compute_event_signature
from relapse.event_auth import validate_event_for_room_version
from relapse.events import EventBase
from relapse.events.snapshot import EventContext, UnpersistedEventContextBase
from relapse.events.validator import EventValidator
from relapse.federation.federation_client import InvalidResponseError
from relapse.handlers.pagination import PURGE_PAGINATION_LOCK_NAME
from relapse.http.servlet import assert_params_in_dict
from relapse.logging.context import nested_logging_context
from relapse.logging.opentracing import RelapseTags, set_tag, tag_args, trace
from relapse.metrics.background_process_metrics import run_as_background_process
from relapse.module_api import NOT_SPAM
...
from relapse.types import JsonDict, StrCollection, get_domain_from_id
...
from relapse.visibility import filter_events_for_server
...
backfill_processing_before_timer = Histogram(
    "relapse_federation_backfill_processing_before_time_seconds",
    "sec",
    [],
    buckets=(
        0.1,
...
class FederationHandler:
    ...
    def __init__(self, hs: "HomeServer"):
        ...
        self._partial_state_syncs_maybe_needing_restart: dict[
            str, tuple[Optional[str], Set[str]]
        ] = {}
    ...
    async def _maybe_backfill_inner(
        ...
        *,
        ...
        set_tag(
            RelapseTags.RESULT_PREFIX + "sorted_backfill_points",
            str(sorted_backfill_points),
        )
        set_tag(
            RelapseTags.RESULT_PREFIX + "sorted_backfill_points.length",
        ...
        set_tag(
            RelapseTags.RESULT_PREFIX + "extremities_to_request",
            str(extremities_to_request),
        )
        set_tag(
            RelapseTags.RESULT_PREFIX + "extremities_to_request.length",
        ...
        async def try_backfill(domains: StrCollection) -> bool:
            ...
            for dom in domains:
                ...
                try:
                    ...
                    continue
                except (RelapseError, InvalidResponseError) as e:
    ...
    async def send_invite(self, target_host: str, event: EventBase) -> EventBase:
        ...
        try:
            ...
            raise RelapseError(502, f"Can't connect to server {target_host}")
    ...
    async def on_invite_request(
        ...
        if event.state_key is None:
            raise RelapseError(400, "The invite event did not have a state key")
        ...
        if self.hs.config.server.block_non_admin_invites:
            raise RelapseError(403, "This server does not accept room invites")
        ...
        if event.type != EventTypes.Member or membership != Membership.INVITE:
            raise RelapseError(400, "The event was not an m.room.member invite event")
        ...
        if not self.is_mine_id(event.state_key):
            raise RelapseError(400, "The invite event must be for this server")

        # block any attempts to invite the server notices mxid
        if event.state_key == self._server_notices_mxid:
            raise RelapseError(HTTPStatus.FORBIDDEN, "Cannot invite this user")
    ...
    async def on_backfill_request(
        ...
        # Relapse asks for 100 events per backfill request. Do not allow more.
        limit = min(limit, 100)
    ...
    async def exchange_third_party_invite(
        ...
        if await self._event_auth_handler.is_host_in_room(room_id, self.hs.hostname):
            ...
            try:
                ...
                raise RelapseError(502, "Failed to forward third party invite")
    ...
    async def _check_signature(self, event: EventBase, context: EventContext) -> None:
        ...
        Raises:
            AuthError: if signature didn't match any keys, or key has been
                revoked,
            RelapseError: if a transient error meant a key couldn't be checked
                for revocation.
        """
    ...
    async def _check_key_revocation(self, public_key: str, url: str) -> None:
        ...
        try:
            ...
            raise RelapseError(502, "Third party certificate could not be checked")
