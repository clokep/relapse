...
from relapse.api.room_versions import KNOWN_ROOM_VERSIONS, RoomVersion, RoomVersions
...
from relapse.federation.federation_client import InvalidResponseError, PulledPduInfo
...
from relapse.replication.http.devices import (
    ReplicationMultiUserDevicesResyncRestServlet,
)
from relapse.replication.http.federation import (
...
from relapse.util.async_helpers import Linearizer, concurrently_execute
from relapse.util.iterutils import batch_iter, partition, sorted_topologically_batched
from relapse.util.retryutils import NotRetryingDestination
from relapse.util.stringutils import shortstr
...
soft_failed_event_counter = Counter(
    "relapse_federation_soft_failed_events_total",
    "Events received over federation that we marked as soft_failed",
)

# Added to debug performance and track progress on optimizations
backfill_processing_after_timer = Histogram(
    "relapse_federation_backfill_processing_after_time_seconds",
    "sec",
    [],
    buckets=(
        0.1,
...
class FederationEventHandler:
    ...
    async def on_send_membership_event(
        ...
        Raises:
            ...
            RelapseError if the event is not accepted into the room
        ...
        if get_domain_from_id(event.sender) != origin:
            ...
            raise RelapseError(403, "User not from origin", Codes.FORBIDDEN)

        if event.sender != event.state_key:
            raise RelapseError(400, "state_key and sender must match", Codes.BAD_JSON)
    ...
    async def process_remote_join(
        ...
        Raises:
            RelapseError if the response is in some way invalid.
        ...
        if create_event is None:
            ...
            raise RelapseError(400, "No create event in state")
        ...
        if room_version.identifier != room_version_id:
            raise RelapseError(400, "Room version mismatch")
        ...
        with nested_logging_context(suffix=event.event_id):
            ...
            await self._check_event_auth(origin, event, context)
            if context.rejected:
                raise RelapseError(403, "Join event was rejected")
    ...
    async def backfill(
        ...
        this method throws a RelapseError.
        ...
        if self._is_mine_server_name(dest):
            raise RelapseError(400, "Can't backfill from self.")
    ...
    async def _process_pulled_events(
        ...
        """
        set_tag(
            RelapseTags.FUNC_ARG_PREFIX + "event_ids",
            str([event.event_id for event in events]),
        )
        set_tag(
            RelapseTags.FUNC_ARG_PREFIX + "event_ids.length",
            str(len(events)),
        )
        ...
        set_tag(
            RelapseTags.RESULT_PREFIX + "new_events.length",
            str(len(new_events)),
        )
        ...
        set_tag(
            RelapseTags.FUNC_ARG_PREFIX + "events_with_failed_pull_attempts",
            str(event_ids_with_failed_pull_attempts),
        )
        set_tag(
            RelapseTags.RESULT_PREFIX + "events_with_failed_pull_attempts.length",
            str(len(events_with_failed_pull_attempts)),
        )
        set_tag(
            RelapseTags.FUNC_ARG_PREFIX + "fresh_events",
            str([event.event_id for event in fresh_events]),
        )
        set_tag(
            RelapseTags.RESULT_PREFIX + "fresh_events.length",
    ...
    async def _get_state_ids_after_missing_prev_event(
        ...
        set_tag(
            RelapseTags.RESULT_PREFIX + "missing_auth_event_ids",
            str(missing_auth_event_ids),
        )
        set_tag(
            RelapseTags.RESULT_PREFIX + "missing_auth_event_ids.length",
            str(len(missing_auth_event_ids)),
        )
        set_tag(
            RelapseTags.RESULT_PREFIX + "missing_desired_event_ids",
            str(missing_desired_event_ids),
        )
        set_tag(
            RelapseTags.RESULT_PREFIX + "missing_desired_event_ids.length",
        ...
        if failed_to_fetch:
            ...
            set_tag(
                RelapseTags.RESULT_PREFIX + "failed_to_fetch",
                str(failed_to_fetch),
            )
            set_tag(
                RelapseTags.RESULT_PREFIX + "failed_to_fetch.length",
        ...
        async def prep(event: EventBase) -> None:
            with nested_logging_context(suffix=event.event_id):
                ...
                try:
                    ...
                    # as rejected, for moderate compatibility with older Relapse
                    # versions.
    ...
    async def _check_event_auth(
        ...
        set_tag(
            RelapseTags.RESULT_PREFIX + "claimed_auth_events",
            str([ev.event_id for ev in claimed_auth_events]),
        )
        set_tag(
            RelapseTags.RESULT_PREFIX + "claimed_auth_events.length",
        ...
        if instance != self._instance_name:
            ...
            if not backfilled:  # Never notify for backfilled events
                with start_active_span("notify_persisted_events"):
                    set_tag(
                        RelapseTags.RESULT_PREFIX + "event_ids",
                        str([ev.event_id for ev in events]),
                    )
                    set_tag(
                        RelapseTags.RESULT_PREFIX + "event_ids.length",
                        str(len(events)),
                    )
                    for event in events:
    ...
    def _sanity_check_event(self, ev: EventBase) -> None:
        ...
        Raises:
            RelapseError if the event does not pass muster
        """
        if len(ev.prev_event_ids()) > 20:
            ...
            raise RelapseError(HTTPStatus.BAD_REQUEST, "Too many prev_events")
        ...
        if len(ev.auth_event_ids()) > 10:
            ...
            raise RelapseError(HTTPStatus.BAD_REQUEST, "Too many auth_events")
