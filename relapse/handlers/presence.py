...
and persisted periodically. When Relapse starts each worker loads the current
...
from relapse.replication.http.presence import (
    ReplicationBumpPresenceActiveTime,
    ReplicationPresenceSetState,
)
from relapse.replication.http.streams import ReplicationGetStreamUpdates
from relapse.replication.tcp.commands import ClearUserSyncsCommand
from relapse.replication.tcp.streams import PresenceFederationStream, PresenceStream
from relapse.storage.databases.main import DataStore
from relapse.storage.databases.main.state_deltas import StateDelta
...
from relapse.util.wheel_timer import WheelTimer
...
notified_presence_counter = Counter("relapse_handler_presence_notified_presence", "")
federation_presence_out_counter = Counter(
    "relapse_handler_presence_federation_presence_out", ""
)
presence_updates_counter = Counter("relapse_handler_presence_presence_updates", "")
timers_fired_counter = Counter("relapse_handler_presence_timers_fired", "")
federation_presence_counter = Counter(
    "relapse_handler_presence_federation_presence", ""
)
bump_active_time_counter = Counter("relapse_handler_presence_bump_active_time", "")

get_updates_counter = Counter("relapse_handler_presence_get_updates", "", ["type"])

notify_reason_counter = Counter(
    "relapse_handler_presence_notify_reason", "", ["locality", "reason"]
)
state_transition_counter = Counter(
    "relapse_handler_presence_state_transition", "", ["locality", "from", "to"]
)
    ...
    async def update_external_syncs_row(  # noqa: B027 (no-op by design)
        ...
        Args:
            process_id: An identifier for the process the users are
                syncing against. This allows relapse to process updates
...
class WorkerPresenceHandler(BasePresenceHandler):
    ...
    async def set_state(
        ...
        if presence not in self.VALID_PRESENCE:
            raise RelapseError(400, "Invalid presence state")
...
class PresenceHandler(BasePresenceHandler):
    def __init__(self, hs: "HomeServer"):
        ...
        LaterGauge(
            "relapse_handlers_presence_user_to_current_state_size",
            "",
            [],
        ...
        LaterGauge(
            "relapse_handlers_presence_wheel_timer_size",
            "",
            [],
    ...
    async def _on_shutdown(self) -> None:
        ...
        earlier than they should when relapse is restarted. This affect of this
    ...
    async def set_state(
        ...
        new_fields: dict[str, str | int | None] = {"state": presence}
    ...
    async def _unsafe_process(self) -> None:
        ...
        while True:
            with Measure(self.clock, "presence_delta"):
                ...
                relapse.metrics.event_processing_positions.labels("presence").set(
                    max_pos
                )
...
def handle_timeouts(
    ...
    syncing_user_devices: Set[tuple[str, str | None]],
...
def handle_timeout(
    ...
    is_mine: bool,
    syncing_device_ids: Set[tuple[str, str | None]],
