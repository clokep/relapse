...
import relapse.events.snapshot
...
from relapse.events.snapshot import UnpersistedEventContext
from relapse.events.utils import copy_and_fixup_power_levels_contents
from relapse.handlers.relations import BundledAggregations
from relapse.rest.admin._base import assert_user_is_admin
...
class RoomCreationHandler:
    ...
    async def _upgrade_room(
        ...
        tombstone_context: relapse.events.snapshot.EventContext,
    ) -> str:
        """
        Args:
            requester: the user requesting the upgrade
    ...
    async def create_room(
        ...
        if not isinstance(room_version_id, str):
            raise RelapseError(400, "room_version must be a string", Codes.BAD_JSON)
        ...
        if "room_alias_name" in config:
            for wchar in string.whitespace:
                if wchar in config["room_alias_name"]:
                    raise RelapseError(400, "Invalid characters in room alias")
            ...
            if mapping:
                raise RelapseError(400, "Room alias already taken", Codes.ROOM_IN_USE)
        ...
        if invite_list or invite_3pid_list:
            try:
                ...
                raise RelapseError(400, "Cannot invite so many users at once")
        ...
        if not allowed_by_third_party_rules:
            raise RelapseError(403, "Room visibility value not allowed.")
    ...
    def _generate_room_id(self) -> str:
        ...
        Relapse's room IDs are 18 [a-zA-Z] characters long, which comes out to around
        102 bits.
...
class TimestampLookupHandler:
    ...
    async def get_event_for_timestamp(
        ...
        Raises:
            RelapseError if unable to find any event locally in the given direction
        """
        logger.debug(
