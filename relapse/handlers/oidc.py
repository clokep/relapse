...
from relapse.config.oidc import OidcProviderClientSecretJwtKey, OidcProviderConfig
from relapse.handlers.sso import MappingException, UserAttributes
...
from relapse.types import JsonDict, UserID, map_username_to_mxid_localpart
from relapse.util import Clock, json_decoder
from relapse.util.caches.cached_call import RetryOnExceptionCachedCall
from relapse.util.macaroons import MacaroonGenerator, OidcSessionData
from relapse.util.templates import _localpart_from_email_filter
...
class OidcHandler:
    ...
    async def handle_oidc_callback(self, request: RelapseRequest) -> None:
        """Handle an incoming request to /_relapse/client/oidc/callback

        Since we might want to display OIDC-related errors in a user-friendly
        way, we don't raise RelapseError from here. Instead, we call
        ...
        if b"error" in request.args:
            ...
            # either the provider misbehaving or Relapse being misconfigured.
    ...
    async def handle_backchannel_logout(self, request: RelapseRequest) -> None:
        """Handle an incoming request to /_relapse/client/oidc/backchannel_logout
        ...
        if logout_token is None:
            raise RelapseError(400, "Missing logout_token in request")
        ...
        try:
            ...
            raise RelapseError(400, "Invalid logout_token in request")
        ...
        try:
            ...
            raise RelapseError(400, "Invalid logout_token payload in request")
        ...
        if oidc_provider is None:
            raise RelapseError(400, "Could not find the OP that issued this event")
...
class OidcProvider:
    ...
    def __init__(
        ...
        self._callback_path_prefix = (
            public_baseurl_path.encode("utf-8") + b"_relapse/client/oidc"
        )
            ...
            if not self._config.backchannel_logout_ignore_sub:
                ...
                try:
                    ...
                    logger.warning(
                        ...
                        "and you want Relapse to ignore the `sub` claim in OIDC "
    ...
    async def handle_redirect_request(
        ...
        parameters:
          ...
          - ``response_type``: ``code``
          - ``redirect_uri``: the callback URL ; ``{base url}/_relapse/client/oidc/callback``
    ...
    async def handle_oidc_callback(
        self, request: RelapseRequest, session_data: "OidcSessionData", code: str
    ) -> None:
        """Handle an incoming request to /_relapse/client/oidc/callback

        By this time we have already validated the session on the relapse side, and
    ...
    def _remote_id_from_userinfo(self, userinfo: UserInfo) -> str:
        ...
        # Some OIDC providers use integer IDs, but Relapse expects external IDs
        # to be strings.
        return str(remote_user_id)

    async def handle_backchannel_logout(
        self, request: RelapseRequest, logout_token: str
    ) -> None:
        """Handle an incoming request to /_relapse/client/oidc/backchannel_logout
        ...
        # This is not that important for now since Relapse is registered
        ...
        # support dynamic registration in Relapse at some point.
        ...
        # Which means the audience should contain Relapse's client_id and the
        ...
        try:
            ...
            raise RelapseError(400, "Invalid logout_token")
