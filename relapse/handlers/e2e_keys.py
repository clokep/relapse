...
from relapse.api.errors import CodeMessageException, Codes, NotFoundError, RelapseError
...
from relapse.logging.opentracing import log_kv, set_tag, tag_args, trace
...
class E2eKeysHandler:
    ...
    async def query_local_devices(
        ...
        for user_id, device_ids in query.items():
            ...
            if not self.is_mine(UserID.from_string(user_id)):
                ...
                set_tag("error", True)
                raise RelapseError(400, "Not a user here")
    ...
    async def upload_signing_keys_for_user(
        ...
        if not master_key:
            raise RelapseError(400, "No master key available", Codes.MISSING_PARAM)
        ...
        try:
            ...
            if "master_key" in keys:
                ...
                raise RelapseError(400, "Invalid master key", Codes.INVALID_PARAM)
            else:
                # the invalid key came from the database
                logger.error("Invalid master key found for user %s", user_id)
                raise RelapseError(500, "Invalid master key")
        ...
        if "self_signing_key" in keys:
            ...
            try:
                ...
                raise RelapseError(400, "Invalid self-signing key", Codes.INVALID_PARAM)
        if "user_signing_key" in keys:
            await self.store.set_e2e_cross_signing_key(
    ...
    async def upload_signatures_for_device_keys(
        ...
        Raises:
            RelapseError: if the signatures dict is not valid.
        """
        # This can only be called from the main process.
        assert isinstance(self.device_handler, DeviceHandler)
    ...
    async def _process_self_signatures(
        ...
        if not isinstance(signatures, dict):
            raise RelapseError(400, "Invalid parameter", Codes.INVALID_PARAM)
    ...
    def _check_master_key_signature(
        ...
        Raises:
            RelapseError: if a signature is invalid
        """
        ...
        for signing_key_id, signature in sigs[user_id].items():
            ...
            if (
                ...
                raise RelapseError(400, "Invalid signature", Codes.INVALID_SIGNATURE)
    ...
    async def _get_e2e_cross_signing_verify_key(
        ...
        Raises:
            ...
            RelapseError: if `user_id` is invalid
        """
...
def _check_device_signature(
    ...
    Raises:
        RelapseError: if the signature was invalid or the sent device is not the
    ...
    if stripped_signed_device != stripped_stored_device:
        ...
        raise RelapseError(400, "Key does not match")
