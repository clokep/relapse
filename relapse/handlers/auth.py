...
from relapse.handlers.ui_auth import (
...
invalid_login_token_counter = Counter(
    "relapse_user_login_invalid_login_tokens",
...
def convert_client_dict_legacy_fields_to_identifier(
    ...
    Raises:
        RelapseError: If the format of the client dict is invalid
    """
    ...
    if not identifier:
        raise RelapseError(400, "Invalid login submission", Codes.INVALID_PARAM)
...
def login_id_phone_to_thirdparty(identifier: JsonDict) -> dict[str, str]:
    ...
    if "country" not in identifier or (
        # The specification requires a "phone" field, while Relapse used to require a "number"
...
class AuthHandler:
    ...
    async def check_ui_auth(
        ...
        relapse.rest.client._base.interactive_auth_handler as a
        decorator.
        ...
        if not sid:
            ...
            # relapse.rest.client.register.RegisterRestServlet.on_POST
            ...
            if session.clientdict != clientdict:
                logger.warning(
                    ...
                    "authentication session. A future version of Relapse "
    ...
    async def _check_auth_dict(
        ...
        Raises:
            ...
            RelapseError if there was a problem with the request
    ...
    async def validate_login(
        ...
        if not isinstance(login_type, str):
            raise RelapseError(400, "Bad parameter: type", Codes.INVALID_PARAM)
        ...
        if login_type == LoginType.PASSWORD:
            ...
            if not passwords_allowed_here:
                raise RelapseError(400, "Password login has been disabled.")
            if not isinstance(password, str):
                raise RelapseError(400, "Bad parameter: password", Codes.INVALID_PARAM)
        ...
        if identifier_dict["type"] == "m.id.thirdparty":
            ...
            if medium is None or address is None:
                raise RelapseError(400, "Invalid thirdparty identifier")

            # For emails, canonicalise the address.
            # We store all email addresses canonicalised in the DB.
            # (See add_threepid in relapse/handlers/auth.py)
            if medium == "email":
                try:
        ...
        if identifier_dict["type"] != "m.id.user":
            raise RelapseError(400, "Unknown login identifier type")

        username = identifier_dict.get("user")
        if not username:
            raise RelapseError(400, "User identifier is missing 'user' key")
    ...
    async def delete_access_token(self, access_token: str) -> None:
        ...
        if not token:
            ...
            raise RelapseError(HTTPStatus.UNAUTHORIZED, "Unrecognised access token")
        await self.store.delete_access_token(access_token)
    ...
    async def add_threepid(
        ...
        # on accounts, so in future Relapse will gain knowledge of specific
        # types (mediums) of threepid. For now, we still use the existing
        # infrastructure, but this is the start of relapse gaining knowledge
        ...
        # Inform Relapse modules that a 3PID association has been created.
        await self._third_party_rules.on_add_user_third_party_identifier(
            user_id, medium, address
        )

        # Deprecated method for informing Relapse modules that a 3PID association
    ...
    async def delete_local_threepid(
        ...
        # Inform Relapse modules that a 3PID association has been deleted.
    ...
    async def start_sso_ui_auth(self, request: RelapseRequest, session_id: str) -> str:
        """
        ...
        if not idps:
            ...
            raise RelapseError(400, "User has no SSO identities")
    ...
    async def get_username_for_registration(
        ...
        for callback in self.get_username_for_registration_callbacks:
            try:
                ...
                raise RelapseError(code=500, msg="Internal Server Error")
