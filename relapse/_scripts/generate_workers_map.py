...
class EndpointDescription:
    ...
    #  - what 'opinionated relapse worker class' (event_creator, synchrotron, etc) does
    #    it go in?
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Updates a relapse database to the latest schema and optionally runs background updates"
            " on it."
        )
    )
    ...
    parser.add_argument(
        ...
        required=True,
        help="Relapse configuration file",
    )
    ...
    worker_paths = get_registered_paths_for_default(
        "relapse.app.generic_worker", config
    )
