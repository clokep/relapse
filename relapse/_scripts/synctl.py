...
from relapse.config import find_config_files

MAIN_PROCESS = "relapse.app.homeserver"
...
one of the following:
 - Either set the environment variable 'RELAPSE_CACHE_FACTOR'
...
def start(pidfile: str, app: str, config_files: Iterable[str], daemonize: bool) -> bool:
    """Attempts to start a relapse main or worker process.
    Args:
        pidfile: the pidfile we expect the process to create
        app: the python module to run
        config_files: config files to pass to relapse
        daemonize: if True, will include a --daemonize argument to relapse
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="whether to start, stop or restart the relapse",
    )
    parser.add_argument(
        "configfile",
        nargs="?",
    ...
    parser.add_argument(
        ...
        " and the main relapse process",
    )
    parser.add_argument(
        "--no-daemonize",
        action="store_false",
        dest="daemonize",
        help="Run relapse in the foreground for debugging. "
    ...
    start_stop_relapse = True

    if cache_factor:
        write(SYNCTL_CACHE_FACTOR_WARNING)
        os.environ["RELAPSE_CACHE_FACTOR"] = str(cache_factor)

    cache_factors = config.get("synctl_cache_factors", {})
    for cache_name, factor in cache_factors.items():
        os.environ["RELAPSE_CACHE_FACTOR_" + cache_name.upper()] = str(factor)
    ...
    if options.all_processes:
        # To start the main relapse with -a you need to add a worker file
        # with worker_app == "relapse.app.homeserver"
        start_stop_relapse = False
    ...
    for worker_configfile in worker_configfiles:
        ...
        if worker_app == "relapse.app.homeserver":
    ...
    if action == "start" or action == "restart":
        ...
        for worker in workers:
            ...
            if worker.cache_factor:
                os.environ["RELAPSE_CACHE_FACTOR"] = str(worker.cache_factor)
